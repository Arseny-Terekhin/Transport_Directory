    ...
    void initialize() {
        ...
        btInsert.setOnAction(actionEvent -> {
            ...
            if (brand.equals("") && model.equals("") && category.equals("")  && number.equals("")){
                ...
                if (bd.numSearch(number)){
                    lbEror.setText("Не удалось добавить так, как транспортное средство с таким нномер уже существует");
                }else {
                    bd.insert(brand, model, category, number, type, date, trailer);
                    openView("resource/Main-View.fxml");
                    btExit.getScene().getWindow().hide();
                }
            }
